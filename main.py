...
# local modules
from input_stream import WebcamVideoStream
from widgets import Timer, SubImage, HistogramPlot, FPS_plot
    ...
    def __init__(self):
        ...
        if self.write_output:
            ...
            self.output_writer = cv2.VideoWriter(
                "output.avi", fourcc, 20.0, (self.cam.w, self.cam.h)
            )
                ...
                for module in self.modules:
                    ...
                    if module.output is not None:
                        resized_overlay = cv2.resize(
                            module.output.copy(), (module.w, module.h)
                        ).astype(np.uint8)

                        output_image[
                            module.y : module.y + module.h,
                            module.x : module.x + module.w,
                        ] = blend_transparent(
                            output_image[
                                module.y : module.y + module.h,
                                module.x : module.x + module.w,
